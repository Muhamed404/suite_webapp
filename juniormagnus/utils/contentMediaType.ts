 ...
 * `/jnr/contents/…` — so the backend's OCI SDK credentials are used
...
export function resolveAwmContentUrl(raw: string | null | undefined): string | null {
  ...
  if (s.startsWith("http://") || s.startsWith("https://")) {
    ...
    try {
      ...
      for (const marker of ["/contents/", "/module_translations/", "/certificates/"]) {
        ...
        if (idx >= 0) {
          // e.g. /jnr/contents/brochures/system_files/file.pdf
          return `/jnr${parsed.pathname.slice(idx)}${parsed.search}`;
        }
      }
    } catch {
  ...
  if (s.startsWith("/jnr")) return s;
  if (s.startsWith("/contents/")) return `/jnr${s}`;
  if (s.startsWith("/module_translations/")) return `/jnr${s}`;
  if (s.startsWith("/certificates/")) return `/jnr${s}`;

  return `/jnr/contents/${s.startsWith("/") ? s.slice(1) : s}`;
}
