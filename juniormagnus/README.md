# Junior Magnus - Security Awareness Management (AWM)

Junior Magnus is a premium, feature-rich Security Awareness Management platform designed to help organizations monitor, manage, and improve their cybersecurity posture through employee training, gamification, and comprehensive risk analytics.
...
```bash
# from the juniormagnus project root
npm install
npm run build
```
...
Create a directory inside the central obfuscated bundle (for example: Secure-Magnus-obfuscated/suite_webapp/juniormagnus) and place or copy the built artifacts into it (.next, public, package.json, README.md, next.config.js, etc.).

- On the target (inside the obfuscated bundle) install production deps and start:

```bash
cd Secure-Magnus-obfuscated/suite_webapp/juniormagnus
npm install
npm start
```
