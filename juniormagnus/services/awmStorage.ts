 ...
 *
 * All asset URLs resolve through the Next.js proxy at /jnr/{type}/...
 * so the browser never needs direct access to the backend.
 */

// ─── Constants ───────────────────────────────────────────────────────

/** Next.js basePath for the juniormagnus app */
export const AWM_BASE_PATH = "/jnr";
 ...
 * Next.js proxy at `/jnr/{assetType}/...`.
 *
 * Handles:
 *  - Absolute backend asset URLs (http/https + /{assetType}/...) → rewritten to /jnr/... proxy
 *  - Other absolute URLs (http/https) → returned as-is
 *  - Already-prefixed paths (/jnr/...) → returned as-is
 *  - Bare video platform domains → https:// prepended
 *  - Paths starting with /{assetType}/ → /jnr prepended
 *  - Bare relative paths → /jnr/{assetType}/ prepended
 *
 ...
 * @returns       - A client-safe URL (e.g. "/jnr/contents/org/1/video.mp4")
 */
export function resolveAssetUrl(
  ...
  // Absolute backend upload URLs should be proxied via /jnr/{type}/...
  ...
  // Bare relative path → prepend /jnr/{type}/
 ...
 * Routes through the proxy at `/jnr/contents/...`
 */
 ...
 * Routes through the proxy at `/jnr/module_translations/...`
 */
 ...
 * Routes through the proxy at `/jnr/certificates/...`
 */
 ...
 * @returns Full URL like "http://localhost:3003/contents/org/1/logo.png"
 */
