...
/** API path prefix per AWM docs: {BASE_URL}/api. Set NEXT_PUBLIC_JNR_API_BASE=/api/jnr if your backend is mounted there. */
export const API_BASE =
  process.env.JNR_API_BASE || process.env.NEXT_PUBLIC_JNR_API_BASE || "/api/jnr";
...
export const AWM_TOKEN_COOKIE = "jnr_session";
...
export const jnrClient = axios.create({
  // When on client, we want usage like jnrClient.get(API_BASE + "/...") to map to /jnr/api/jnr/...
  // Since API_BASE is /api/jnr, we just need the basepath /jnr as the baseURL.
  // But wait, if we set baseURL to "/jnr", then get("/api/jnr/...") becomes "/jnr/api/jnr/...". Correct.
  baseURL: typeof window === "undefined" ? SERVICE_JNR_URL : "/jnr",
  withCredentials: true,
});

export const suiteClient = axios.create({
  // Suite calls are like post("/login"). We want them to go to /jnr/api/suite/login.
  baseURL: typeof window === "undefined" ? SERVICE_SUITE_URL : "/jnr/api/suite",
  withCredentials: true,
});

setupInterceptors(jnrClient);
