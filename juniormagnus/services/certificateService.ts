...
export const certificateService = {
   ...
   * API: GET /api/jnr/certificate
   */
  getCertificates: async (params?: { lang_id?: number; org_id?: number; filter?: string }) => {
    return request<CertificateTemplate[]>(() =>
      jnrClient.get<jnrResponseBody>(`${API_BASE}/certificate`, { params })
    );
  },

  /**
   * Get certificate template by ID.
   * API: GET /api/jnr/certificate/:id
   */
  getCertificateById: async (id: number) => {
    return request<CertificateTemplate>(() =>
      jnrClient.get<jnrResponseBody>(`${API_BASE}/certificate/${id}`)
    );
  },

  /**
   * Create a new certificate template.
   * API: POST /api/jnr/certificate
   */
  createCertificate: async (formData: FormData, orgId?: number) => {
    return request<CertificateTemplate>(() =>
      jnrClient.post<jnrResponseBody>(`${API_BASE}/certificate`, formData, {
   ...
   * API: PUT /api/jnr/certificate/:id
   */
  updateCertificate: async (id: number, formData: FormData, orgId?: number) => {
    return request<CertificateTemplate>(() =>
      jnrClient.put<jnrResponseBody>(`${API_BASE}/certificate/${id}`, formData, {
        params: orgId !== undefined ? { org_id: orgId } : {},
        headers: { "Content-Type": "multipart/form-data" },
      })
    );
   ...
   * API: DELETE /api/jnr/certificate/:id
   */
  deleteCertificate: async (id: number) => {
    return request<null>(() => jnrClient.delete<jnrResponseBody>(`${API_BASE}/certificate/${id}`));
  },

  /**
   * Download certificate template record.
   * API: GET /api/jnr/certificate/download
   */
  downloadTemplate: async (moduleId: number, langId?: number) => {
    return request<CertificateTemplate>(() =>
      jnrClient.get<jnrResponseBody>(`${API_BASE}/certificate/download`, {
