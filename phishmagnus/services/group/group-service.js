...
async function getGroupsByOrganization(req, opts = {}) {
  try {
    ...
    if (!orgId) {
      ...
      logger.warn('[GroupService] missing orgId in session', { path: req?.originalUrl, user: req?.user?.userId });
    ...
    logger.info(`[GroupService] fetching groups ${url}`);
    ...
    logger.error('[GroupService] error fetching groups', { error: error && (error.stack || error.message) });
