    ...
    if (req.method === "GET") {
      ...
      try {
        ...
        const [
          ...
          TemplateService.listTemplatesByOrgAndType(organization, enums.phishingType.SMS, req).catch(err => ({ data: { message: [] }, _err: err })),
          TemplateService.getSystemTemplates(enums.phishingType.SMS, req).catch(err => ({ _err: err, templates: null }))
        ]);
        ...
        return res.render(render_ejs_urls.PhishMagnus.Campaign.SMS.CREATE, {
...
exports.submitFormController = async (req, res) => {
  logger.info("Controller - Create SMS Campaign:Incoming request");
  try {
    if (req.method === "POST") {
      ...
      const url = backend_api_urls.PHISHMAGNUS.CAMPAIGN.SMS.CREATE;
      ...
      if (response.data.success) {
        ...
        logger.info(`Controller - Create SMS Campaign: Created new campaign id=${newCampaign.id}, name=${newCampaign.name}`);
        logger.info(`Controller - Create SMS Campaign: Redirecting to campaign details page for campaign id=${newCampaign.id}`);
        return res.redirect(frontend_api_urls.PHISHMAGNUS.Campaign.SMS.VIEW(newCampaign.id));
      }
