...
exports.getUsersByGroup = async (req, res) => { 
  try {
    logger.info("Incoming request for getUsersByGroup with params: " + JSON.stringify(req.params, null, 2));
    ...
    logger.info('Printing assigned users for group ' + groupId + ' and organization ' + organization + ': ' + JSON.stringify(users, null, 2));
    return res.status(200).json({ success: true, assignedUsers: users });

  } catch (error) {
    logger.error("Exception in assign users by group: " + error.stack);
