...
async function enrolToGroup(req, res) {
    try {
        ...
        logger.info("Incoming request to enrolToGroup: SELECTED USERS " + JSON.stringify(selectedUsers, null, 2));
        ...
        logger.info('Printing backend api url in enrolToGroup ' + url);
        ...
        logger.info('Incoming request to enrolToGroup: POST ENROL TO GROUP ' + JSON.stringify(response.data, null, 2));
        return res.status(200).json({ success: true, unassignedUsers: message });

    } catch (error) {
        logger.error("Incoming request to enrolToGroup: " + error.stack);
        req.flash('alertType', 'error');
