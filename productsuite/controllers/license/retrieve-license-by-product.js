...
exports.retrieveLicenseInformationByProductEnumKey = async (req, res) => {
    logger.info(`[Retrieve License By Product Enum Key]: Incoming Params ${JSON.stringify(redactLogData(req.params), null, 2)}`);
    try {
        ...
        logger.info(`[Retrieve License By Product Enum Key]: URL ${redactLogData(url)}`);
        const response = await apiClient.get(url);

        logger.info(`[Retrieve License By Product Enum Key]: Response from API ${JSON.stringify(redactLogData(response.data), null, 2)}`);
        const { message, object } = response.data;
        logger.info(`[Retrieve License By Product Enum Key]: Message: ${redactLogData(message)}, Object: ${JSON.stringify(redactLogData(object), null, 2)}`);
        ...
        logger.info(`[Retrieve License By Product Enum Key]: Filtered License: ${JSON.stringify(redactLogData(filteredLicense), null, 2)}`);
        ...
        logger.error(`[Retrieve License By Product Enum Key]: Error occurred - ${redactLogData(error.message)}`);
