...
exports.listMFAMails = async (req, res) => {
  logger.info(`Controller - [MFAMail]: query=${JSON.stringify(req.query)}`);
  ...
  try {
    ...
    logger.info(`Controller - [MFAMail]: Calling backend API: ${url}`);
    ...
    if (!response?.data?.success) {
      logger.error(`Controller - [MFAMail]: Backend failure: ${JSON.stringify(response?.data)}`);
    ...
    logger.error(`Controller - [MFAMail]: ${error.message}`);
