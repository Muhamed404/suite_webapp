...
exports.deleteUser = async (req, res, next) => {
  logger.info(`Controller - Delete User: Incoming request to delete user with ID ${req.params.userId}`);
  try {
    const userIdToDelete = parseInt(req.params.userId, 10);
    if (isNaN(userIdToDelete) || userIdToDelete <= 0) {
      logger.warn(`Controller - Delete User: Invalid user ID passed: ${req.params.userId}`);
    ...
    const response = await apiClient.delete(deleteUrl);
      logger.info(`Controller - Delete User: Response from API: ${JSON.stringify(response.data, null, 2)}`);
    if (response.data.success) {
      ...
      logger.error(`Controller - Delete User: Failed to delete user. Message: ${response.data.message}`);
      req.flash("alertType", "error");
    ...
    logger.error(`Error - Controller - Delete User: ${error}`);
