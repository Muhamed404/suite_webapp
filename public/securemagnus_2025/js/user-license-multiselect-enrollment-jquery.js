...
function moveToUnenrolled() {
  ...
  if (movedIds.length && window.unEnrollUserLicense) {
    window.unEnrollUserLicense(movedIds)
      .then(() => {
    ...
    function moveToEnrolled() {
      ...
      if (movedIds.length && window.enrollUserLicense) {
        window.enrollUserLicense(movedIds)
          .then(() => {
