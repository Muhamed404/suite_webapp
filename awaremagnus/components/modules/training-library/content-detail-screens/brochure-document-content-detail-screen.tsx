...
interface BrochureDocumentContentDetailScreenProps {
  moduleId: number;
...
export function BrochureDocumentContentDetailScreen({
  ...
  const documents = (allContents as ModuleContent[])
    .filter((c) => c.content_type_id === contentTypeId)
    .sort((a, b) => (a.order ?? 0) - (b.order ?? 0) || a.id - b.id);
  const currentIndex = documents.findIndex((c) => c.id === contentId);
  const nextDoc =
    currentIndex >= 0 && currentIndex < documents.length - 1 ? documents[currentIndex + 1] : null;
  const nextDocHref = nextDoc
    ? `${basePath}/${moduleId}/content/${contentTypeId}/${nextDoc.id}`
    : null;
  const otherDocs = documents.filter((c) => c.id !== contentId);
  ...
  const nextSectionLabelKey = isBrochure
    ? "library.nextBrochureTraining"
    : "library.nextDocumentTraining";
              ...
              <div>
                <div className="flex flex-col">
                  ...
                  <div className="bg-white rounded-xl overflow-hidden">
                    <div className="min-h-[500px]" style={{ minHeight: "800px" }}>
                      {isLoading ? (
                  ...
                  {otherDocs.length > 0 && (
                    <div className="mt-4 bg-white rounded-xl p-4">
                      <h5 className="text-sm font-semibold mb-3 text-gray-900">
                        {t(nextSectionLabelKey) ??
                          (isBrochure ? "Next Brochure Training" : "Next Document Training")}
                      </h5>
                      <div className="space-y-3">
                        {otherDocs.slice(0, 10).map((doc: ModuleContent) => (
                          <DocumentListItem
                            key={doc.id}
                            basePath={basePath}
...
function DocumentListItem({
  ...
  const detailHref = `${basePath}/${moduleId}/content/${contentTypeId}/${doc.id}`;
  const logoUrl = getContentAssetUrl(doc.logo_url ?? doc.logo_path);
  const fallbackIcon = getContentTypeIconFor(doc.content_type_id, typeLabel);
  ...
  return (
    ...
    >
      {logoUrl ? (
        <div className="relative flex-shrink-0 w-12 h-12 rounded-lg overflow-hidden bg-gray-200">
