...
export const GamificationStats = () => {
  ...
  const totalAchievements = stats?.total_achievements || 0;
  const unlockedAchievements = stats?.total_unique_achievements_unlocked || 0;
  const totalUniqueAchievements = (stats?.total_unique_achievements_unlocked || 0) + (stats?.total_unique_achievements_locked || 0);

  const achievementPercentage = totalUniqueAchievements > 0 ? Math.round((unlockedAchievements / totalUniqueAchievements) * 100) : 0;
        ...
        */}
        <div className="col-span-3 row-span-1 bg-white rounded-xl p-4 flex flex-col justify-between">
          <div className="flex items-center gap-2 text-sm text-gray-500 font-medium">
            <Image src="/images/gard_cap.svg" alt="" width={28} height={28} className="text-lg" />
            <div>
        ...
        <div className="col-span-3 col-start-4 row-span-1 bg-white rounded-xl p-4 flex flex-col justify-start">
          <div className="flex items-center gap-2 text-sm text-gray-500 font-medium">
            <Image src="/images/clock_icon.svg" alt="" width={28} height={28} className="text-lg" />
            <div>
        ...
        <div className="col-span-6 col-start-7 row-span-3 bg-[linear-gradient(114.67deg,#FFFEFC_5.61%,#FDECE0_98.45%)] rounded-xl p-5">
          <div className="flex items-start justify-between">
            <div>
              <h2 className="font-semibold text-gray-900 text-lg flex items-center gap-2">
                <Image src="/images/img/Icon_Trophy.svg" alt="" width={28} height={28} className="text-lg" />
                {t("gamification.achievementGallery")}
              </h2>
              <p className="text-gray-500 text-sm mt-1.5">{t("gamification.achievementSubtitle")}</p>
            </div>
            <Link href="#" className="text-blue-600 text-sm font-medium">{t("cards.viewAll")}</Link>
          </div>
          ...
          <div className="grid grid-cols-8 gap-4 gap-y-5 mt-10">
            {stats?.achievement_statistics?.slice(0, 15).map((ach, i) => (
              <div key={i} className="w-14 h-14 rounded-full flex items-center justify-center relative group">
                {/* Use the image URL from API if valid, else placeholder */}
                {ach.image_small_url ? (
                  <Image
                    src={`/images/achivement/${ach.image_small_url}`}
                    onError={(e) => { (e.target as HTMLImageElement).src = '/images/achivement/1.png' }}
                    alt={ach.achievement_name}
                    width={56} height={56}
                    className="w-14 h-14"
                  />
                ) : (
                  <Image src={`/images/achivement/${(i % 5) + 1}.png`} alt="" width={56} height={56} className="w-14 h-14" />
                )}
            ...
            {(!stats?.achievement_statistics || stats.achievement_statistics.length === 0) && (
              <div className="col-span-8 text-center text-gray-400 text-sm">No achievements found</div>
            )}
          </div>
          ...
          <div className="mt-12 p-5 bg-white rounded-lg">
            <div className="flex justify-between text-gray-500 text-sm font-medium mb-2">
              ...
              <div>
                <span className="text-gray-700 text-xl">{unlockedAchievements}/</span>
                <span className="text-gray-700 text-base">{totalUniqueAchievements}</span>
              </div>
            </div>
            <div className="w-full h-2.5 bg-gray-200 rounded-full">
              <div className="h-2.5 bg-purple-500 rounded-full" style={{ width: `${achievementPercentage}%` }}></div>
            </div>
          </div>
        </div>
            ...
            <div className="grid grid-cols-4 gap-5 flex-1 pl-5 border-l border-[#E6E6E6]">
              {avatars?.avatar_statistics?.map((level, index) => (
                ...
                >
                  ...
                  <p className="text-xs leading-tight text-gray-700 mt-2 w-[90%] truncate">{level.level_name}</p>
                </div>
              ))}
            </div>
          </div>
